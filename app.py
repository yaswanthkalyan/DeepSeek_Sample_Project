...
if "message_log" not in st.session_state:
    ...
    st.session_state.message_log.append({"role": "ai", "content": "Hi! I'm DeepSeek. How can I assist with your coding today? 💻"})
...
def build_prompt_chain():
    """Constructs a dynamic prompt sequence maintaining conversation history."""
    prompt_sequence = [system_prompt]  

    for msg in st.session_state.message_log:
        if msg["role"] == "user":
            prompt_sequence.append(HumanMessagePromptTemplate.from_template("{content}").format(content=msg["content"]))
        elif msg["role"] == "ai":
            prompt_sequence.append(AIMessagePromptTemplate.from_template("{content}").format(content=msg["content"]))
